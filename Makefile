...
$(TEST): test/binding.test.c lib$(LANGUAGE_NAME).a | $(MAKE_BUILD_DIR)
	$(CC) $(CPPFLAGS) $(CFLAGS) -Ibindings/c $< lib$(LANGUAGE_NAME).a $(LDFLAGS) $(LDLIBS) -o $@

install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/sh '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/sh
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/sh
